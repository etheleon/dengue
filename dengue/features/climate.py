...
def get_temp_weekly(schema="national_analysis", table="temperature", window=12, lag=0) -> pd.DataFrame:
    ...
    This function queries a database to retrieve daily temperature data, calculates the mean-centered maximum temperature at the weekly level,
    ...
    query = f"""--sql
      ...
      averaged AS (
        SELECT
          year,
          week,
          AVG(temperature_max_mean_centered) OVER (ORDER BY year, week ROWS BETWEEN {window - 1} PRECEDING AND CURRENT ROW) AS max_t_scale_{window}_wk_avg
        FROM scaled
	  )
      SELECT
		year,
		week,
    	LAG(max_t_scale_{window}_wk_avg, {lag}, 0) OVER (ORDER BY year, week) AS max_t_scale_{window}_wk_avg_{lag}
	  FROM averaged
    """
...
def get_elnino34_ssta_weekly(schema="national_analysis", table="elnino34", window=12, lag=4) -> pd.DataFrame:
    ...
    query = f"""--sql
      ...
      SELECT
        year,
        week,
        LAG(nino34_{window}_wk_avg, {lag}, 0) OVER (ORDER BY year, week) AS nino34_{window}_wk_avg_{lag}
      FROM averaged
    """
